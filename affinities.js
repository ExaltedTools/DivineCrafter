...
function poec_initAffinities(){
    if($("#poecAffinities").length==0){
        ...
        setTimeout(function(){poec_affInitLoad();},1);
    }
}
...
function poec_affInitLoad(){
    ...
    */
    ...
    if(poeaf_options["omode"]=="m"){var s1="selected";var s2="";}else{var s1="";var s2="selected";}
    ...
    if(poeaf_options["archs"]==0){var addsel=" selected";}else{var addsel="";}
    vHTML+='<div class="choice none '+addsel+'">None<div class="value">0</div></div>';
    $.each(poeexd["supports"],function(key,val){
        if(poeaf_options["archs"]==val){var addsel=" selected";}else{var addsel="";}
    ...
    if(poeaf_options["archk"]==0){var addsel=" selected";}else{var addsel="";}
    vHTML+='<div class="choice none '+addsel+'">None<div class="value">0</div></div>';
    $.each(poeexd["keystones"],function(key,val){
        if(poeaf_options["archk"]==val){var addsel=" selected";}else{var addsel="";}
    ...
    if(poeaf_options["mtbt"]==0){var addsel=" selected";}else{var addsel="";}
    vHTML+=  '<div class="choice all '+addsel+'">All<div class="value">0</div></div>';
    if(poeaf_options["mtbt"]=="offence"){var addsel=" selected";}else{var addsel="";}
    vHTML+=  '<div class="choice offence '+addsel+'">Offence<div class="value">offence</div></div>';
    if(poeaf_options["mtbt"]=="defence"){var addsel=" selected";}else{var addsel="";}
    vHTML+=  '<div class="choice defence '+addsel+'">Defence<div class="value">defence</div></div>';
    if(poeaf_options["mtbt"]=="other"){var addsel=" selected";}else{var addsel="";}
    ...
    $("#poecAffOptions.aff").find(".mcui-checkbox").mcuiCheck({"change":function(){poec_affSetOptions(false);}});
    $("#poecAffOptions.aff").find(".mcui-radio").mcuiRadio({"change":function(){poec_affSetOptions(false);}});
...
function poec_affGetIniCheck(val){
    if(val==1){return "ischecked";}else{return "unchecked";}
}
...
function poec_initAffSearchFilter(){
    ...
    $("#poecAffFilterInput").keyup(function(){poec_affSearchFilterGO();}).focus(function(){
...
function poec_affBuildMSelector(source,idf,namef,sid,title,multi,addcls,invalide,selected,ct){
    ...
    for(var i=0;i<source.length;i++){
        var acls="";
        var valid=true;
        if(addcls){if(addcls[source[i][idf]]){var acls=addcls[source[i][idf]];}}
        if(invalide&&acls==""){valid=false;}
        if(valid){
            if(cursel.indexOf("|"+source[i][idf]+"|")>-1){var sel=" selected";}else{var sel="";}
...
function poec_affUpdateResult(){
    ...
    setTimeout(function(){poec_affUpdateResultGO();},1);
}
...
function poec_affUpdateResultGO(){
    ...
    if(hasninja){
        var ninjaget="";
        if(ninjaprm["class"]!=""){ninjaget+="&class="+ninjaprm["class"];}
        if(ninjaprm["skill"]!=""){ninjaget+="&skill="+ninjaprm["skill"];}
        if(ninjaprm["keystone"]!=""){ninjaget+="&keystone="+ninjaprm["keystone"];}
    ...
    if(uarr){
        for(var i=0;i<uarr.length;i++){
            if(!uprev["asc"][poeaf["undata"][uarr[i]]["c"]]){uprev["asc"][poeaf["undata"][uarr[i]]["c"]]=0;}
            uprev["asc"][poeaf["undata"][uarr[i]]["c"]]++;
            if(!uprev["skl"][poeaf["undata"][uarr[i]]["s"]]){uprev["skl"][poeaf["undata"][uarr[i]]["s"]]=0;}
            uprev["skl"][poeaf["undata"][uarr[i]]["s"]]++;
            if(!uprev["bas"][poeaf["undata"][uarr[i]]["b"]]){uprev["bas"][poeaf["undata"][uarr[i]]["b"]]=0;}
    ...
    if(aarr){
        for(var i=0;i<aarr.length;i++){
            if(!nprev["asc"][poeaf["andata"][aarr[i]]["c"]]){nprev["asc"][poeaf["andata"][aarr[i]]["c"]]=0;}
            nprev["asc"][poeaf["undata"][aarr[i]]["c"]]++;
            if(!nprev["skl"][poeaf["andata"][aarr[i]]["s"]]){nprev["skl"][poeaf["andata"][aarr[i]]["s"]]=0;}
    ...
    if(parr){
        ...
        for(var i=0;i<parr.length;i++){
            if(!prevalence["bas"][poeaf["njdata"][parr[i]]["b"]]){prevalence["bas"][poeaf["njdata"][parr[i]]["b"]]=0;}
            prevalence["bas"][poeaf["njdata"][parr[i]]["b"]]++;
            if(!prevalence["asc"][poeaf["njdata"][parr[i]]["c"]]){prevalence["asc"][poeaf["njdata"][parr[i]]["c"]]=0;}
            prevalence["asc"][poeaf["njdata"][parr[i]]["c"]]++;
            if(!prevalence["skl"][poeaf["njdata"][parr[i]]["s"]]){prevalence["skl"][poeaf["njdata"][parr[i]]["s"]]=0;}
            prevalence["skl"][poeaf["njdata"][parr[i]]["s"]]++;
            if(!prevalence["bit"][poeaf["njdata"][parr[i]]["i"]]){prevalence["bit"][poeaf["njdata"][parr[i]]["i"]]=0;}
        ...
        if(nmst["skl"]!==null&&nmst["skl"]!==0){vSTOUT+="<div class='skl'><div>"+nmst["skl"]+"</div><div>"+poeexd["gems"]["seq"][poeexd["gems"]["ind"][skl]]["name_gem"]+"</div></div>";ntog++;}
        if(nmst["slt"]!==null&&nmst["slt"]!==0){vSTOUT+="<div class='slt'><div>"+nmst["slt"]+"</div><div>"+poeexd["slots"]["seq"][poeexd["slots"]["ind"][slt]]["name"]+"</div></div>";ntog++;}
        if(nmst["bas"]!==null&&nmst["bas"]!==0){vSTOUT+="<div class='bas'><div>"+nmst["bas"]+"</div><div>"+poecd["bases"]["seq"][poecd["bases"]["ind"][bas]]["name_base"]+"</div></div>";ntog++;}
        if(nmst["bit"]!==null&&nmst["bit"]!==0){vSTOUT+="<div class='bit'><div>"+nmst["bit"]+"</div><div>"+poecd["bitems"]["seq"][poecd["bitems"]["ind"][bit]]["name_bitem"]+"</div></div>";ntog++;}
        if(nmst["ars"]!==null&&nmst["ars"]!==0){vSTOUT+="<div class='ars'><div>"+nmst["ars"]+"</div><div>"+archsname+"</div></div>";ntog++;}
        if(nmst["ark"]!==null&&nmst["ark"]!==0){vSTOUT+="<div class='ark'><div>"+nmst["ark"]+"</div><div>"+archkname+"</div></div>";ntog++;}
        if(ntog>1){
        ...
        if(poeaf_options["omode"]=="m"){
            ...
            for(var i=0;i<affar.length;i++){
                if(affinds[affar[i]["id"]]){
                    for(var j=0;j<affinds[affar[i]["id"]].length;j++){
                        var valid=true;
                        if(poeaf_options["crafted"]==0){if(idmgrp==poeaf_constants["crafted_mgrp"]){valid=false;}}
                        if(poeaf_options["essences"]==0){if(idmgrp==poeaf_constants["essence_mgrp"]){valid=false;}}
                        if(poeaf_options["droponly"]==0){if(poeaf_constants["droponly_mgrps"].indexOf("|"+idmgrp+"|")>-1){valid=false;}}
                        if(poeaf_options["influences"]==0){if(poeaf_constants["influence_mgrps"].indexOf("|"+idmgrp+"|")>-1){valid=false;}}
                        if(valid){
                ...
                if(affinds[affar[i]["id"]]){
                    for(var j=0;j<affinds[affar[i]["id"]].length;j++){
                        if(poecd["modifiers"]["seq"][poecd["modifiers"]["ind"][affinds[affar[i]["id"]][j]["i"]]]!=undefined){
                            ...
                            if(poeaf_options["droponly"]==0){if(poeaf_constants["droponly_mgrps"].indexOf("|"+idmgrp+"|")>-1){valid=false;}}
                            if(poeaf_options["influences"]==0){if(poeaf_constants["influence_mgrps"].indexOf("|"+idmgrp+"|")>-1){valid=false;}}
                            if(affar[i]["val"]/pretotval[aftype]<poeaf_constants["mtresh"]){valid=false;}
                            if(valid){
                    ...
                    for(var j=0;j<affinds[affar[i]["id"]].length;j++){
                        if(poecd["modifiers"]["seq"][poecd["modifiers"]["ind"][affinds[affar[i]["id"]][j]["i"]]]!=undefined){
                            ...
                            if(idmgrp==1){
                                ...
                                var aftype=poecd["modifiers"]["seq"][poecd["modifiers"]["ind"][affinds[affar[i]["id"]][j]["i"]]]["affix"];
                                if(affar[i]["val"]/pretotval[aftype]<poeaf_constants["mtresh"]){valid=false;}
                                if(affinds[affar[i]["id"]][j]["t"]=="p"){var checkid=affinds[affar[i]["id"]][j]["i"]+affinds[affar[i]["id"]][j]["t"]+affinds[affar[i]["id"]][j]["m"];}else{var checkid=affinds[affar[i]["id"]][j]["i"];}
                                if(idsparsed.indexOf("|"+checkid+"|")>-1){valid=false;}else{idsparsed+=checkid+"|";}
                                if(poeaf_constants["ignores"].indexOf("|"+affinds[affar[i]["id"]][j]["i"]+"|")>-1){valid=false;}
                                if(valid){
                                    numvalaff++;
                                    var modgroup=poecd["modifiers"]["seq"][poecd["modifiers"]["ind"][affinds[affar[i]["id"]][j]["i"]]]["modgroups"];
                                    var mname=poecl["mod"][affinds[affar[i]["id"]][j]["i"]];
                                    ...
                                    for(var zy=0;zy<modgroup.length;zy++){
                                        ...
                                        if(mmgacheck[aftype][modgroup[zy]].indexOf("|"+mname+"|")>-1){}else{
                        ...
                        if(poecd["modifiers"]["seq"][poecd["modifiers"]["ind"][affinds[affar[i]["id"]][j]["i"]]]!=undefined){
                            ...
                            if(valid){
                                ...
                                if(poeaf_constants["childs_mgrps"].indexOf("|"+idmgrp+"|")>-1&&affinds[affar[i]["id"]][j]["w"]==0&&affbindex[aftype][mname]!=undefined){
                                    ...
                                    if(maffar[aftype][affbindex[aftype][mname]]["ccheck"].indexOf("|"+idmgrp+"|")>-1){}else{
            ...
            if(numvalaff>0){
                ...
                var arrMTypes=[];
                $.each(mtypes,function(mkey,mval){arrMTypes.push({"id":mkey,"val":mval});});
                arrMTypes.sort(sortColByVAL);
                arrMTypes.reverse();
                var arrMTByOffence=[];
                $.each(mtbt["offence"],function(mkey,mval){arrMTByOffence.push({"id":mkey,"val":mval});});
                arrMTByOffence.sort(sortColByVAL);
                arrMTByOffence.reverse();
                var arrMTByDefence=[];
                $.each(mtbt["defence"],function(mkey,mval){arrMTByDefence.push({"id":mkey,"val":mval});});
                arrMTByDefence.sort(sortColByVAL);
                arrMTByDefence.reverse();
                var arrMTByOther=[];
                $.each(mtbt["other"],function(mkey,mval){arrMTByOther.push({"id":mkey,"val":mval});});
...
function poec_affCleanMTypes(marr){
    ...
    $.each(marr,function(mkey,mval){
        if(poeaf_constants["mtbt_excludes"].indexOf("|"+mkey+"|")>-1){}else{
            narr[mkey]=mval;
        }
    });
    return narr;
    ...
    if(poeaf_options["mgrouped"]==1){
        for(var j=0;j<gaff.length;j++){
            ...
            for(var i=0;i<maff[gaff[j]["id"]].length;i++){
                ...
                if(chkpmod.indexOf("|"+mgrp+"-"+poecl["mod"][maff[gaff[j]["id"]][i]["mid"]]+"|")>-1){}else{
                    var addcls="";
                    var addpick="";
                    if(bas&&picked<maxpicks&&!mgpicked){
                        ...
                        if(mconflict||(affgblcrafted&&mgrp==poeaf_constants["crafted_mgrp"])){}else{
                            var avalid=true;
                            if(poeaf_constants["influence_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                if(affgblsinfs.indexOf("|"+mgrp+"|")>-1){}else{
                                    if(affgblninfs<2){
                                        affgblninfs++;
                                        affgblsinfs+=mgrp+"|";
                                    }else{
                            ...
                            if(avalid){
                                if(mgrp==poeaf_constants["crafted_mgrp"]){affgblcrafted=true;}
                                for(var zy=0;zy<modgroup.length;zy++){
                    ...
                    if(maff[gaff[j]["id"]][i]["val"]<1){maff[gaff[j]["id"]][i]["val"]=1;}
        ...
        for(var i=0;i<maff.length;i++){
            ...
            if(chkpmod.indexOf("|"+mgrp+"-"+poecl["mod"][maff[i]["mid"]]+"|")>-1){}else{
                var addcls="";
                var addpick="";
                if(bas&&picked<maxpicks){
                    ...
                    if(mconflict||(affgblcrafted&&mgrp==11)){}else{
                        var avalid=true;
                        if(poeaf_constants["influence_mgrps"].indexOf("|"+mgrp+"|")>-1){
                            if(affgblsinfs.indexOf("|"+mgrp+"|")>-1){}else{
                                if(affgblninfs<2){
...
function poec_affOutputMtCats(mtcats){
    var vRet="";
    if(mtcats["other"]){vRet+="<div class='mtcat other'>Utility</div>";}
    if(mtcats["defence"]){vRet+="<div class='mtcat defence'>Defence</div>";}
    if(mtcats["offence"]){vRet+="<div class='mtcat offence'>Offence</div>";}    
    return vRet;
}
...
function poec_affApplyAffixHovers(){
    $("#poecAffResByType").find(".affix:not(.modgroup)").hover(function(){
        ...
        if(!wbyaffid[aid]){wbyaffid[aid]=0;}
        ...
        if(lpower.length>0){
            ...
            for(var i=0;i<tpower.length;i++){
                if(tpower.length==1){ht="TOT";}else{ht="P"+(i+1);};
                hovHTML+="<div>"+ht+"</div>";
            }
            if(tpower.length>1){
                hovHTML+="<div>TOT</div>";
        ...
        if(aftype=="suffix"){
            ...
            $("#poecBindGrpToolTip").css({"left":thispos.left+$(this).outerWidth(),"right":"auto","top":thispos.top});
        }
        if(hovHTML!=""){
...
function sortColByVAL(a,b){if(a["val"]===b["val"]){return 0;}else{return (a["val"] < b["val"]) ? -1 : 1;}}
function sortColByNAME(a,b){if(a["name"] < b["name"]) { return -1; }if(a["name"] > b["name"]) { return 1; }return 0;}
...
function poec_initAppraise(){
    if($("#poecAppraise").length==0){
        ...
        setTimeout(function(){poec_appInitLoad()},1);
    }
}
        ...
        $.each(njdat["a"],function(amkey,amcnt){
            if(njdat["b"]!="na"){
                ...
                //if(poeaf_constants["noskillid"].indexOf("|"+njdat["s"]+"|")>-1){}else{
                    if(poecap["tops"][poecap["rvslot"][njdat["b"]]]==undefined){poecap["tops"][poecap["rvslot"][njdat["b"]]]={"asc":{},"skl":{},"cmb":{}};}
                    if(poecap["tops"][poecap["rvslot"][njdat["b"]]]["asc"][njdat["c"]]==undefined){poecap["tops"][poecap["rvslot"][njdat["b"]]]["asc"][njdat["c"]]={};}
                    if(poecap["tops"][poecap["rvslot"][njdat["b"]]]["skl"][njdat["s"]]==undefined){poecap["tops"][poecap["rvslot"][njdat["b"]]]["skl"][njdat["s"]]={};}
                    if(poecap["tops"][poecap["rvslot"][njdat["b"]]]["cmb"][cmbk]==undefined){poecap["tops"][poecap["rvslot"][njdat["b"]]]["cmb"][cmbk]={};}
                    if(poecap["bmpart"][amkey]==undefined){poecap["bmpart"][amkey]={};}
                    if(poecap["bmpart"][amkey][poecap["rvslot"][njdat["b"]]]==undefined){poecap["bmpart"][amkey][poecap["rvslot"][njdat["b"]]]={};}
                    if(poecap["bmpart"][amkey][poecap["rvslot"][njdat["b"]]][cmbk]==undefined){poecap["bmpart"][amkey][poecap["rvslot"][njdat["b"]]][cmbk]=0;}                    
                    ...
                    if(poeaf["affind"]["base"][njdat["b"]][amkey]){
                        for(var j=0;j<poeaf["affind"]["base"][njdat["b"]][amkey].length;j++){
                            ...
                            if(poecap["tops"][poecap["rvslot"][njdat["b"]]]["asc"][njdat["c"]][mmkey]==undefined){poecap["tops"][poecap["rvslot"][njdat["b"]]]["asc"][njdat["c"]][[mmkey]]=0;}
                            if(poecap["tops"][poecap["rvslot"][njdat["b"]]]["skl"][njdat["s"]][mmkey]==undefined){poecap["tops"][poecap["rvslot"][njdat["b"]]]["skl"][njdat["s"]][[mmkey]]=0;}
                            if(poecap["tops"][poecap["rvslot"][njdat["b"]]]["cmb"][cmbk][mmkey]==undefined){poecap["tops"][poecap["rvslot"][njdat["b"]]]["cmb"][cmbk][[mmkey]]=0;}
...
function poec_appOrderTops(sarr,maxaff,rtype,slotkey){
    $.each(sarr,function(skey,srecs){
        ...
        var curafbt={"prefix":0,"suffix":0,"byflt":{
        ...
        var tarr={"prefix":[],"suffix":[],"tpow":0,"byflt":{
            ...
            if(poecd["modifiers"]["seq"][poecd["modifiers"]["ind"][marr[i]["key"]]]!=undefined){   
                ...
                if(curafbt[atype]<maxafbt){
                    ...
                    if(mconflict){}else{
                        var pass=true;
                        if(poeaf_constants["influence_mgrps"].indexOf("|"+mgrp+"|")>-1){
                            if(strinf.indexOf("|"+mgrp+"|")>-1){}else{
                                if(curinf.length>=2){pass=false;}else{curinf.push(mgrp);strinf+=mgrp+"|";}
                            }
                        }else{
                            if(poeaf_constants["droponly_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                if(hasdrop){pass=false;}else{hasdrop=true;}
                            }
                        }
                        if(pass){
                            curaffs++;
                ...
                if(curafbt["byflt"]["nores"][atype]<maxafbt){
                    ...
                    if(!hasres){
                        ...
                        if(mconflict){}else{
                            var pass=true;
                            if(poeaf_constants["influence_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                if(strinfbyflt["nores"].indexOf("|"+mgrp+"|")>-1){}else{
                                    if(curinfbyflt["nores"].length>=2){pass=false;}else{curinfbyflt["nores"].push(mgrp);strinfbyflt["nores"]+=mgrp+"|";}
                                }
                            }else{
                                if(poeaf_constants["droponly_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                    if(hasdropbyflt["nores"]){pass=false;}else{hasdropbyflt["nores"]=true;}
                                }
                            }
                            if(pass){
                                curabyflt["nores"]++;
                ...
                if(curafbt["byflt"]["noatt"][atype]<maxafbt){
                    ...
                    if(!hasatt){
                        ...
                        if(mconflict){}else{
                            var pass=true;
                            if(poeaf_constants["influence_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                if(strinfbyflt["noatt"].indexOf("|"+mgrp+"|")>-1){}else{
                                    if(curinfbyflt["noatt"].length>=2){pass=false;}else{curinfbyflt["noatt"].push(mgrp);strinfbyflt["noatt"]+=mgrp+"|";}
                                }
                            }else{
                                if(poeaf_constants["droponly_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                    if(hasdropbyflt["noatt"]){pass=false;}else{hasdropbyflt["noatt"]=true;}
                                }
                            }
                            if(pass){
                                curabyflt["noatt"]++;
                ...
                if(curafbt["byflt"]["noall"][atype]<maxafbt){
                    ...
                    if(!hasatt&&!hasres){
                        ...
                        if(mconflict){}else{
                            var pass=true;
                            if(poeaf_constants["influence_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                if(strinfbyflt["noall"].indexOf("|"+mgrp+"|")>-1){}else{
                                    if(curinfbyflt["noall"].length>=2){pass=false;}else{curinfbyflt["noall"].push(mgrp);strinfbyflt["noall"]+=mgrp+"|";}
                                }
                            }else{
                                if(poeaf_constants["droponly_mgrps"].indexOf("|"+mgrp+"|")>-1){
                                    if(hasdropbyflt["noall"]){pass=false;}else{hasdropbyflt["noall"]=true;}
                                }
                            }
                            if(pass){
                                curabyflt["noall"]++;
...
function poec_appImportDone(){
    ...
    $("#poecAppItem").find(".affix.taff").click(function(){
        ...
        if(refresh){
            setTimeout(function(){poec_appAppraise();},10);
        }
    });
...
function poec_appAppraise(){
    ...
    if(!$("#poecAffFilterHolder.app").is(":visible")){
        ...
        $("#poecAffOptions.app").find(".mcui-checkbox").mcuiCheck({"change":function(){poec_appImportDone();}});
    }
        ...
        if(marr!=null){
            for(var j=0;j<marr.length;j++){
                if(crapp_affids.indexOf("|"+marr[j]["i"]+"|")>-1){
                    ...
                    if(crapp_appmcor[marr[j]["i"]].indexOf("|"+modcor+"|")>-1){}else{
                    ...
                    if(strallmodp.indexOf("|"+modcor+"|")>-1){}else{
                        strallmodp+=modcor+"|";
    ...
    for(var i=0;i<crapp_allmods.length;i++){
        $.each(poecap["bmpart"][crapp_allmods[i]][islot],function(cmbkey,cmbcnt){
            if(crapp_apppnts["cmb"][cmbkey]==undefined){crapp_apppnts["cmb"][cmbkey]=0;}
            var scmb=cmbkey.split("-");
            if(crapp_apppnts["asc"][scmb[0]]==undefined){crapp_apppnts["asc"][scmb[0]]=0;}
            if(crapp_apppnts["skl"][scmb[1]]==undefined){crapp_apppnts["skl"][scmb[1]]=0;}
    ...
    $.each(crapp_apppnts["asc"],function(key,val){reordarr["asc"].push({"key":key,"val":0,"prv":0,"score":val,"power":0,"max":0});});
    $.each(crapp_apppnts["skl"],function(key,val){reordarr["skl"].push({"key":key,"val":0,"prv":0,"score":val,"power":0,"max":0});});
    $.each(crapp_apppnts["cmb"],function(key,val){reordarr["cmb"].push({"key":key,"val":0,"prv":0,"score":val,"power":0,"max":0});});
    ...
    // Sort
    reordarr["asc"].sort(sortColByVAL);reordarr["asc"].reverse();
    reordarr["skl"].sort(sortColByVAL);reordarr["skl"].reverse();
    reordarr["cmb"].sort(sortColByVAL);reordarr["cmb"].reverse();
    ...
    $("#poecAppResult").find(".affix").click(function(){poec_appHLSel(this);});
...
function poec_appGoToAff(rtype,rkey){
    ...
    switch(rtype){
        case 'asc' : fparam["asc"]=rkey;break;
        case 'skl' : fparam["skl"]=rkey;break;
        case 'cmb' : 
            rkey=rkey.split("-");
            fparam["asc"]=rkey[0];
            fparam["skl"]=rkey[1];
...
function poec_appHLSel(vThis){
    ...
    for(var i=0;i<crapp_allmods.length;i++){
        ...
        $.each(poecap["bmpart"][crapp_allmods[i]][crapp_slot],function(cmbkey,cmbcnt){
            var scmb=cmbkey.split("-");
            switch(rtype){
                case 'asc' : if(scmb[0]==rkey){ampow+=cmbcnt;} break;
                case 'skl' : if(scmb[1]==rkey){ampow+=cmbcnt;} break;
                case 'cmb' : if(cmbkey==rkey){ampow+=cmbcnt;} break;
            }
        });
    ...
    for(var j=0;j<crapp_item["crsets"]["affixes"].length;j++){
        if(crapp_item["crsets"]["affixes"][j]["toggled"]){
            bampow.push({"key":crapp_item["crsets"]["affixes"][j]["id"],"val":crapp_item["crsets"]["affixes"][j]["ampow"]});
        }
    }
    bampow.sort(sortColByVAL);bampow.reverse();
    ...
    ratord.sort(sortColByVAL);ratord.reverse();
...
function poec_appOutputResult(rdata,rtype){
    ...
    var rtitle="Title";
    switch(rtype){
        case 'asc' : rtitle=applyLang("By Ascendency"); break;
        case 'skl' : rtitle=applyLang("By Skill Gem"); break;
        case 'cmb' : rtitle=applyLang("By Skill Gem/Ascendency"); break;
    }
...
function poec_affTradeWindow(){
    if($("#poecAffTradeWindow").length==0){
        ...
        $("#poecAffTradeWindow").find(".mcui-radio").mcuiRadio({change:function(){poec_affTradeRefreshSelection();}});
    }
    ...
    $("#poecAffTradeWOutput").find(".handle").mousedown(function(e){
        ...
        $(window).mouseup(function(e){
            ...
            if(cv!=iv){$("#poecAffTradeWOutput").find(".handling").addClass("changed");}else{$("#poecAffTradeWOutput").find(".handling").removeClass("changed");}
...
function poec_affTradeRefreshSelection(){
    ...
    var ignoring=[];
    switch(pool){
        case 'life' : ignoring=["Energy Shield","Ward"]; break;
        case 'es' : ignoring=["Life","Ward"]; break;
        case 'ward' : ignoring=["Life","Energy Shield"]; break;
    }
    var nactive=0;
    var totv=0;
...
function poec_affTradeLink(){
    ...
    var wendmod=1;
    switch(slt){
        case 'Amulet' : item="accessory.amulet"; break;
        case 'Ring' : item="accessory.ring"; break;
        case 'Belt' : item="accessory.belt"; wendmod=1.5; break;
        case 'BodyArmour' : item="armour.chest"; wendmod=1.5; break;
        case 'Boots' : item="armour.boots"; wendmod=1.5; break;
        case 'Gloves' : item="armour.gloves"; wendmod=1.5; break;
        case 'Helm' : item="armour.helmet"; wendmod=1.5; break;
        case 'Flask' : item="flask"; naffixes=2; break;
        case 'Cluster' : item="jewel.cluster"; break;
        case 'Jewel' : item="jewel.base"; naffixes=4; break;
        case 'Weapon' : item="weapon"; break;
        default: break;
    }
    ...
    if(bas){
        switch(bas){
            // Armours
            case '49' : armour=[1,0,1,0]; break;
            case '46' : armour=[0,1,0,0]; break;
            case '50' : armour=[0,1,1,0]; break;
            case '48' : armour=[1,1,0,0]; break;
            case '45' : armour=[1,0,0,0]; break;
            case '47' : armour=[0,0,1,0]; break;
            case '51' : armour=[1,1,1,0]; break;
            // Boots
            case '43' : armour=[1,0,1,0]; break;
            case '42' : armour=[1,1,0,0]; break;
            case '39' : armour=[1,0,0,0]; break;
            case '44' : armour=[0,1,1,0]; break;
            case '41' : armour=[0,0,1,0]; break;
            case '40' : armour=[0,1,0,0]; break;
            case '201': armour=[0,0,0,1]; break;
            // Helmets
            case '56' : armour=[1,0,1,0]; break;
            case '55' : armour=[1,1,0,0]; break;
            case '52' : armour=[1,0,0,0]; break;
            case '57' : armour=[0,1,1,0]; break;
            case '54' : armour=[0,0,1,0]; break;
            case '53' : armour=[0,1,0,0]; break;
            case '203': armour=[0,0,0,1]; break;
            // Gloves
            case '37' : armour=[1,0,1,0]; break;
            case '36' : armour=[1,1,0,0]; break;
            case '33' : armour=[1,0,0,0]; break;
            case '38' : armour=[0,1,1,0]; break;
            case '35' : armour=[0,0,1,0]; break;
            case '34' : armour=[0,1,0,0]; break;
            case '202': armour=[0,0,0,1]; break;
            // Shields
            case '9' : armour=[1,0,1,0]; item="armour.shield"; break;
            case '8' : armour=[1,1,0,0]; item="armour.shield"; break;
            case '5' : armour=[1,0,0,0]; item="armour.shield"; break;
            case '10' : armour=[0,1,1,0]; item="armour.shield"; break;
            case '7' : armour=[0,0,1,0]; item="armour.shield"; break;
            case '6' : armour=[0,1,0,0]; item="armour.shield"; break;
            case '212' : item="armour.shield"; type="Bone Spirit Shield"; break;
            // Quiver
            case '4' : item="armour.quiver"; wendmod=1.5; break;
            // Weapons
            case '17' : item="weapon.sceptre"; break;
            case '18' : item="weapon.wand"; break;
            case '19' : item="weapon.runedagger"; break;
            case '21' : item="weapon.basestaff"; break;
            case '15' : item="weapon.oneaxe"; break;
            case '20' : item="weapon.bow"; break;
            case '11' : item="weapon.claw"; break;
            case '67' : item="weapon.wand"; type="Convoking Wand"; break;
            case '12' : item="weapon.basedagger"; break;
            case '16' : item="weapon.basemace"; break;
            case '13' : item="weapon.onesword"; break;
            case '14' : item="weapon.onemelee"; break;
            case '24' : item="weapon.twoaxe"; break;
            case '23' : item="weapon.twomace"; break;
            case '22' : item="weapon.twosword"; break;
            case '25' : item="weapon.warstaff"; break;
            // Jewels
            case '31' : case '30' : case '32' : case '29' : item="jewel.abyss"; break;
        }
    }
    ...
    */
    ...
    if(armour){
        ...
        if(armour[0]){filters["ar"]["min"]=1;}else{filters["ar"]["max"]=0;}
        if(armour[1]){filters["ev"]["min"]=1;}else{filters["ev"]["max"]=0;}
        if(armour[2]){filters["es"]["min"]=1;}else{filters["es"]["max"]=0;}
        if(armour[3]){filters["ward"]["min"]=1;}else{filters["ward"]["max"]=0;}
    ...
    for(var i=0;i<maxaff;i++){
        ...
        if(curraffinds[naffar[i]["id"]]){
            for(var j=0;j<curraffinds[naffar[i]["id"]].length;j++){
                ...
                if(curraffinds[naffar[i]["id"]][j]["mp"]>high){high=curraffinds[naffar[i]["id"]][j]["mp"];}
                if(curraffinds[naffar[i]["id"]][j]["mp"]>highest){highest=curraffinds[naffar[i]["id"]][j]["mp"];}
                if(curraffinds[naffar[i]["id"]][j]["mp"]<lowest){lowest=curraffinds[naffar[i]["id"]][j]["mp"];}
            }
        }
        if(high==0){lowest=1;high=1;}
